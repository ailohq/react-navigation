...
export default function useNavigationBuilder(createRouter, options) {
  ...
  const routerOptions = _objectSpread(_objectSpread({}, rest), (route === null || route === void 0 ? void 0 : route.params) && route.params.initial !== false && typeof route.params.screen === 'string' ? {
    initialRouteName: route.params.screen
  } : null);

  const {
    current: router
  } = React.useRef(createRouter(routerOptions));
  const {
    initialRouteName
  } = routerOptions;
