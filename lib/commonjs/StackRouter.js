...
function StackRouter() {
  const router = _objectSpread(_objectSpread({}, _BaseRouter.default), {}, {
    ...
    getInitialState({
      ...
      const initialRouteName = _initialRouteName !== undefined && routeNames.includes(_initialRouteName) ? _initialRouteName : routeNames[0];
      return {
        stale: false,
        type: 'stack',
    ...
    getRehydratedState(partialState, {
      ...
      if (routes.length === 0) {
        const initialRouteName = _initialRouteName !== undefined ? _initialRouteName : routeNames[0];
        routes.push({
          key: "".concat(initialRouteName, "-").concat((0, _nonSecure.nanoid)()),
          name: initialRouteName,
          params: routeParamList[initialRouteName]
